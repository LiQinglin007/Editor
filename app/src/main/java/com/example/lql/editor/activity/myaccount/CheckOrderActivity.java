...
import com.example.lql.editor.adapter.me.CheckOrderAdapter;
...
public class CheckOrderActivity extends Activity implements View.OnClickListener {
    ...
    private android.support.v7.widget.RecyclerView checkorderre;

    CheckOrderAdapter mCheckOrderAdapter;//查重订单适配器

    ArrayList<CheckOrderBean.DataBean> mList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pDialog = new ProgressDialog(CheckOrderActivity.this);
    ...
    private void initView() {
        ...
        this.checkorderre = (RecyclerView) findViewById(R.id.checkorder_re);
        ...
        checkorderre.setLayoutManager(new LinearLayoutManager(this));
        mCheckOrderAdapter = new CheckOrderAdapter(mList, this);
//        checkorderre.addItemDecoration(new DividerItemDecoration(this, MyDecoration.VERTICAL_LIST));
        checkorderre.setAdapter(mCheckOrderAdapter);
        mCheckOrderAdapter.setOnItemClickLitener(new OnItemClickLitener() {
            @Override
            public void onItemClick(View view, int position) {
                PublicStaticData.Ordertype = "0";
                PublicStaticData.OrderId = mList.get(position).getMyServiceId_() + "";
                startActivity(new Intent(CheckOrderActivity.this, OrderDetailsActivity.class));
            }
        });
        ...
        checkorderre.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
                ...
                if (dy != 0) {
                    if (isSlideToBottom(recyclerView)) {
                        if (Count > 0) {
                            ...
                            Toast.makeText(CheckOrderActivity.this, "没有更多内容了", Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            }
    ...
    private void getData(final boolean IsNeedClear, final int index) {
        String Userid = "";
...
//        1:待检测 2：检测中 3： 已完成
        SendRequest.CheckRepeat(Userid, type, Page, Rows, new mOkCallBack() {
            @Override
            public void onSuccess(String response) {
                Log.e("responseCheckRepeat",response);
                pDialog.hide();
             ...
             try{
                CheckOrderBean mCheckOrderBean = JSON.parseObject(response, CheckOrderBean.class);
                if (mCheckOrderBean.getResultCode() == 0) {
                    Count = mCheckOrderBean.getCount();
                    if (IsNeedClear) {
                        mList.clear();
                    }
                    mList.addAll(mCheckOrderBean.getData());
                    mCheckOrderAdapter.notifyDataSetChanged();
                } else {
                    T.shortToast(getApplicationContext(), mCheckOrderBean.getMsg());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void registBroadcastReceiver() {
        ...
        filter.addAction("com.lql.getDataCheckOrder");
    ...
    private BroadcastReceiver mFreshReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("com.lql.getDataCheckOrder".equals(action)) {
                getData(true,1);
            }
        }
    };

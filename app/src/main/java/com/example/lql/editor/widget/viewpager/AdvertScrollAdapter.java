...
//    private String[] id;
    ...
    public AdvertScrollAdapter(Context context, List<View> views, List<GetImglist.DataBean> adverts) {
        mContext = context;
        imgUrls = new String[adverts.size()];
        urls = new String[adverts.size()];
//        id = new String[adverts.size()];
        for (int i = 0; i < adverts.size(); i++) {
            imgUrls[i] = adverts.get(i).getImgurl();
//            id[i]=adverts.get(i).id;
    ...
    public Object instantiateItem(ViewGroup view, final int position) {
        ...
        final String imgUrl = MyUrl.Pic+imgUrls[positionForOthersExceptAdvertView];
...
//            SSLContext sc = SSLContext.getInstance("SSL");
            sc.init(null, new TrustManager[]{new HttpUtils.TrustAllCerts()}, new SecureRandom());

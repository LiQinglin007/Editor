package com.example.lql.editor.widget.pickerview.lib;
...
public class WheelTime {
    ...
    //// TODO: 2016/12/26     这是最后的日
    public static void setEND_Day(int eND_DAY){
        END_DAY=eND_DAY;
    }


    //// TODO: 2016/12/26   设置最后月
    public static void setEND_MON(int eND_MON){
        END_MON=eND_MON;
    }
...
//            wv_month.setAdapter(new NumericWheelAdapter(1, 12));
        wv_month.setAdapter(END_MON==0?new NumericWheelAdapter(1, 12):new NumericWheelAdapter(1, END_MON));
        ...
        OnWheelChangedListener wheelListener_year = new OnWheelChangedListener() {
            ...
            public void onChanged(WheelView wheel, int oldValue, int newValue) {
                ...
                if(year_num!=END_YEAR){
                    ...
                    wv_month.setAdapter(new NumericWheelAdapter(1, END_MON));

                    if (list_big
                            .contains(String.valueOf(wv_month.getCurrentItem() + 1))) {

                        if(wv_month.getCurrentItem() + 1!=END_MON){
                            wv_day.setAdapter(new NumericWheelAdapter(1, 31));
                            maxItem = 31;
                        }else{
                            wv_day.setAdapter(new NumericWheelAdapter(1,END_DAY));
...
//                        }
        ...
        /**
         * 在监听里边写的判断    年份不动   只滑动月份的时候
         *
         *
         *
         * 判断这个月份是不是传进来的最后的月份，如果是，就把最后的时间设置成传进来的天
         *                                       如果不是，就把最后的天设置成按大小月或者润年来设置
         *
         *
         *
         *
        ...
        OnWheelChangedListener wheelListener_month = new OnWheelChangedListener() {
            ...
            public void onChanged(WheelView wheel, int oldValue, int newValue) {
                ...
                if (list_big.contains(String.valueOf(month_num))) {
                    if(month_num!=END_MON&&myyear!=END_YEAR){
                        wv_day.setAdapter(new NumericWheelAdapter(1, 31));
                        maxItem = 31;
                    }else{
                        wv_day.setAdapter(new NumericWheelAdapter(1,END_DAY));
                    ...
                    if(month_num!=END_MON&&myyear!=END_YEAR){
                        ...
                        if (((wv_year.getCurrentItem() + START_YEAR) % 4 == 0 && (wv_year
                                .getCurrentItem() + START_YEAR) % 100 != 0)
                                || (wv_year.getCurrentItem() + START_YEAR) % 400 == 0) {
                            wv_day.setAdapter(new NumericWheelAdapter(1, END_MON));
                            maxItem = END_MON;
                        } else {
                            wv_day.setAdapter(new NumericWheelAdapter(1, END_MON));
                            maxItem = END_MON;
